---
description: Use scripts/git-ai.sh to commit and push changes instead of manual git add/commit/push
alwaysApply: true
---

# git-ai.sh — commit + push automático

## Qué hace

El script está en `scripts/git-ai.sh` (versionado en este repositorio).
...
Usar `bash scripts/git-ai.sh` siempre que el usuario pida hacer commit y push de los cambios, en lugar de la secuencia manual `git add` + `git commit` + `git push`.
...
- El usuario pide hacer `git add` de archivos puntuales (no todo) → stagear manualmente y preguntar antes de usar el script.
